...
authenticator = IAMAuthenticator('IvN00HN58QEdRm3IR4ArSeKcjX5Ov8bsJMyOV6ZXUBnU')
...
authenticator = IAMAuthenticator('0qTkvVfjZFq5xnHR_VS4OFQmpJtbobgh5yUnPsCS99_3')
...
def post_list(request):
    ...
    for post in posts:
        ...
        tone_analysis = tone_analyzer.tone({'text': text}, context_type='application/json').get_result()
        post.analysis = json.dumps(tone_analysis, indent=2)
        post.doctone = json.loads(post.analysis)
        post.tone = post.doctone["document_tone"]
        the_tones = post.tone["tones"]
        post.scores = the_tones
